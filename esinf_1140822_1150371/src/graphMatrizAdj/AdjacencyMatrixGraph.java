...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    public Iterable<V> directConnections(V vertex) {
        if (checkVertex(vertex) == false) return null;
        ArrayList<V> v = new ArrayList<>();
        int index = toIndex(vertex);
        for (int i = 0; i < numVertices; i++) if (edgeMatrix[index][i] != null) v.add(vertices.get(i));
        return (Iterable<V>) v;
    }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        ...
        int index = toIndex(vertex);
        for (int i = 0; i < numVertices; i++) if (edgeMatrix[index][i] != null) edges.add(edgeMatrix[index][i]);
        return (Iterable<E>) edges;
    }
